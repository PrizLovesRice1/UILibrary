...
function Library:AddContextMenu(
    ...
    function Table:Open()
        ...
        -- Fade in + scale animation (0.9 → 1)
        Menu.BackgroundTransparency = 1
        local OpenTween = TweenService:Create(Menu, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            BackgroundTransparency = 0,
        })
        OpenTween:Play()
    ...
    function Table:Close()
        ...
        -- Fade out animation
        local CloseTween = TweenService:Create(Menu, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            BackgroundTransparency = 1,
        })
        CloseTween:Play()
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            Button.Base.MouseEnter:Connect(function()
                ...
                -- Fade in text + soft glow effect on hover
                Button.Tween = TweenService:Create(Button.Base, Library.TweenInfo, {
                    TextTransparency = 0,
                    BackgroundColor3 = Library:GetDarkerColor(Library.Scheme.MainColor),
                })
                Button.Tween:Play()
                
                -- Add slight stroke glow on hover
                if Button.Stroke then
                    local GlowTween = TweenService:Create(Button.Stroke, Library.TweenInfo, {
                        Transparency = 0.3,
                    })
                    GlowTween:Play()
                end
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                -- Click animation: shrink + bounce back
                local ClickTween = TweenService:Create(Button.Base, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = Button.Base.Size - UDim2.fromOffset(4, 4),
                })
                ClickTween:Play()
                ClickTween.Completed:Connect(function()
                    if not Button.Disabled then
                        local BounceTween = TweenService:Create(Button.Base, TweenInfo.new(0.15, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {
                            Size = Button.Base.Size,
                        })
                        BounceTween:Play()
                    end
                end)
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        function Toggle:Display()
            ...
            -- Smooth fade for label text
            TweenService:Create(Label, Library.TweenInfo, {
                TextTransparency = Toggle.Value and 0 or 0.4,
            }):Play()
            
            -- Smooth fade for checkbox image with scale effect
            local CheckTween = TweenService:Create(CheckImage, TweenInfo.new(Library.TweenInfo.Time, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ImageTransparency = Toggle.Value and 0 or 1,
            })
            CheckTween:Play()

            -- Color shift on checkbox on toggle
            local CheckboxColorTween = TweenService:Create(Checkbox, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function Library:CreateWindow(WindowInfo)
    ...
    function Library:Toggle(Value: boolean?)
        ...
        local ToggleTween = TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            BackgroundTransparency = Library.Toggled and 0 or 1,
        })
        ToggleTween:Play()
